...
var nicEditors = {
 ...
 allTextAreas: function(C,textArea1, textArea2) {
  var A = document.getElementsByTagName("textarea");
  var panelID=0;
  for (var B = 0; B < A.length; B++) {
    panelID=B;
    var C;
    if (A[B].id == textArea1 || A[B].id == textArea2) {
      nicEditors.editors.push(new nicEditor(C).panelInstance(A[B], C, panelID));
    }
  }
...
var nicEditor = bkClass.extend({
 ...
 addInstance: function(B, C, panelID) {
  B = this.checkReplace($BK(B));
  if (B.contentEditable || !!window.opera) {
   var A = new nicEditorInstance(B, C, this, panelID)
  } else {
...
var nicEditorInstance = bkClass.extend({
 isSelected: false,
 construct: function(G, D, C, panelID) {
  this.ne = C;
  this.elm = this.e = G;
  this.options = D || {};
 ...
 init: function(panelID) {
  ...
  this.elm.setAttribute("id", "niceEditCtl_"+String(panelID));
  this.elm.setAttribute("value", "default Value");
  
  //this.elm.setAttribute("oninput","wordTyped('niceEditCtl')");
